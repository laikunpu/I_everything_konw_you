...
public class DaoImpl<T> implements IDao<T> {
	...
	public List<T> findByCondition(Class<T> c, Map<String, String> map,
			int first, int max) {
		StringBuffer hql = new StringBuffer("from " + SSH2Uitil.getTableName(c)
				+ " as a ");
		Query query = null;
		List<T> list = null;
		if (null != map) {
	...
	public List<T> getListByCondition(Class<T> c, String field,
		...
		String hql = "from " + SSH2Uitil.getTableName(c) + " as a order by a."
				+ field + " " + Condition;
		// String hql = "from " + SSH2Uitil.getTableName(c);
	...
	public int getCount(Class<T> c, String field, String name) {
		// TODO Auto-generated method stub
		String hql = "select count(*) from " + SSH2Uitil.getTableName(c);
		Query query = null;
